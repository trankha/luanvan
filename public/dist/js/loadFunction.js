...
function loadDefaultData(conf,token,secu,sock,stationid){
	var address;
	var pondid = -1;
	var notf = arrStationName[stationid];
	var regionname = "";
...
function loadFIRST(conf,token,secu,userid,sock){
	...
	$.ajax(settings).done(function (resultdata) {
		...
		loadRealTime(conf,token,secu,userid,sock); /*Goi ham realtime thông báo*/
	}).complete(function(){
...
function loadRealTime(conf,token,secu,userid,sock){
	...
	getStationByUserId(conf,token,secu,userid,function(arrayStation){
		...
		loadDefaultData(config,token,security,sock,arrStation2[0]); /*Chờ api để kiểm tra ở trang chủ mới gọi hàm*/
	...
	jQuery.ajax({
		...
		success: function(resultdata){
			resultdata.data.forEach(function(data,index){
				...
				html += '<i class="fa fa-users text-yellow"></i>' + data.notif_title;
				html += '<p><i class="fa fa-warning text-yellow"></i>Thời gian đo: ';
...
function showBlockStation(config,token,security,userid){
	...
	getStationByUserId(config,token,security,userid,function(arrayStation){
		arrayStation.forEach(function(items){
	        ...
	        html += items.station_name;
			if(arrBlockStation.indexOf(items.station_id) == -1){
				html += '<input type="checkbox" id="cbKhoaTram" name="cbKhoaTram" value="' + items.station_id + '" class="pull-right" onchange="processStationblock(this,'+items.station_id+')">';
			}
			else{
				html += '<input type="checkbox" id="cbKhoaTram" name="cbKhoaTram" checked value="' + items.station_id + '" class="pull-right" onchange="processStationblock(this,'+items.station_id+')">';
			}
			html +='</label>';
	        html +='</div>';
		});
...
/*HÀM TRẢ VỀ DANH SÁCH LỜI KHUYÊN*/
function getListAdvice(config,token,security){
	var td = "";
    var arrayUserId = [];
    var arrayThresholdId = [];
    var arrayAdviceId = [];
    var arrayAdviceMessage = [];
    var arrayAdviceCreateDate = [];
    queue.push(1);
	jQuery.ajax({
		url: config + '/api/advice/getall',
  		headers:{
  		...
  		success: function(resultdata){
  			resultdata.data.forEach(function(items){
	  			arrayUserId.push(items.user_id);
	  			arrayThresholdId.push(items.threshold_id);
	  			arrayAdviceId.push(items.advice_id);
	  			arrayAdviceMessage.push(items.advice_message);
	  			arrayAdviceCreateDate.push(items.advice_createdDate);
  			});
  		},
  		error: function(jqXHR,textStatus,errorThrown){
		...
		queue.pop();
		if(queue.length == 0){
			for(i in arrayAdviceId){
				td += "<tr>";
				td += "<td>" + arrayUserId[i] + "</td>";
		        td += "<td>" + arrayThresholdId[i] + "</td>";
		        td += "<td>" + arrayAdviceMessage[i] + "</td>";
		        td += "<td>" + moment(arrayAdviceCreateDate[i]).utc().format('DD-MM-YYYY, HH:mm') + "</td>";
		        td += "<td>";
		        td += "<a href='/chuyengia/loikhuyen/capnhatloikhuyen/" +arrayAdviceId[i]+"' title='cập nhật'>";
...
function getUserInfoByUserId(config,token,security,userid,callback){
	var data;
	queue.push(1);
	jQuery.ajax({
		url: config + '/api/user/getbyid/' + userid,
  		headers:{
	  		'Content-Type' : 'application/x-www-form-urlencoded',
  			'authorization': security + token
  		},
  		...
  		error: function(jqXHR,textStatus,errorThrown){
			displayError("Lỗi ! Không thể tải dữ liệu của người dùng theo id. Vui lòng tải lại trang");
		},
	}).complete(function(){
		queue.pop();
		if(queue.length == 0){
...
/*HÀM CHO TRẠM*/
/*Hàm lấy danh sách trạm theo userid*/
function getStationByUserIdForListStation(config,token,security,userid){
	queue3.push(1);
	var arrayStationId = [];
	jQuery.ajax({
		url : config + '/api/station/getbyuser/' + userid,
		...
		success: function(resultdata){
			resultdata.data.forEach(function(items){
				arrayStationId.push(items.station_id);
			});
		},
		error: function(jqXHR,textStatus,errorThrown){
			displayError("Lỗi ! Không thể lấy danh sách trạm theo userid. Vui lòng tải lại trang");
		...
		queue3.pop();
		if(queue3.length == 0){
			getStationByArrayId(config,token,security,arrayStationId)
		}
	});
}
// var arrayStationData = [];
var td = '';
/*Hàm lấy  thông tin trạm theo array station id*/
function getStationByArrayId(config,token,security,arrayStationId){
	arrayStationId.forEach(function(items){
		queue2.push(1);
		getStationById(config,token,security,items);
	});
}
/*Hàm lấy  thông tin trạm theo station id*/
function getStationById(config,token,security,StationId){
	jQuery.ajax({
		...
		success: function(items){
			td += '<tr>';
			td += '<td>' + items.data.station_name + '</td>';
			td += '<td>' + items.data.region_id + '</td>';
			td += '<td>' + items.data.station_address + '</td>';
			td += '<td>' + items.data.station_status + '</td>';
			td += '<td>' + items.data.station_duration + '</td>';
			td += '</tr>';
			// arrayStationData.push({station_id:items.data.station_id,sink_id:items.data.sink_id,region_id:items.data.region_id,river_id:items.data.river_id,pond_id:items.data.pond_id,station_name:items.data.station_name,station_location:items.data.station_location,station_node:items.data.station_node,station_status:items.data.station_status,station_address:items.data.station_address,station_duration:items.data.station_duration});
			// arrayStationData.push({station_id:items.data.station_id,sink_id:items.data.sink_id,region_id:items.data.region_id});
		},
		error: function(jqXHR,textStatus,errorThrown){
...
/*HÀM CHƯA TEST CỦA VIEW THEMTRAM*/
// var arrayReg = [];
// //ham lay ve danh sach vung dua tren id user
...
// 	});
// }
/*****************************CÁC HÀM TẠM KHÓA KHÔNG SỬ DỤNG NỮA***************************/
...
// 		if(queue.length == 0){
// 			var _objListenerForStation;
// 			console.log("DS trạm " + arrStation2[0]);
// 			loadDefaultData(conf,token,secu,socket_global,arrStation2[0]);
