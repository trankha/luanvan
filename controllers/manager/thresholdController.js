...
thresholdController.get("/danhsachnguong",service.ensureAuthenticated, function(req, res) {
    ...
    var totalStudents = 0,
      ...
      totalStudents = thresholdData.length;
  		res.render("manager/nguong/danhsachnguong", {title: 'Xem danh sách ngưỡng',navi:"nguong  >  xemdanhsachnguong",thresholdData:thresholdData,secu:config.securitycode,conf:config.urladdress,token:req.session.token,userid:req.session.userid,username:req.session.username});
  	});
});
thresholdController.get("/themnguong",service.ensureAuthenticated,function(req,res){
    res.render("manager/nguong/themnguong",{title: 'Thêm ngưỡng',secu:config.securitycode,conf:config.urladdress,token:req.session.token,userid:req.session.userid,username:req.session.username})
});
thresholdController.post("/themnguong",function(req,res){
    ...
    service.post(options,function(error,body){
      ...
      res.redirect('/quanly/nguong/danhsachnguong');
    });
});
thresholdController.get("/capnhatnguong/:id",service.ensureAuthenticated,function(req,res){
    ...
    service.get(options,function(error,data){
      if(error){
        ...
        res.render("manager/nguong/capnhatnguong",{title: 'Cập nhật ngưỡng',secu:config.securitycode,conf:config.urladdress,token:req.session.token,userid:req.session.userid,username:req.session.username,thresholdData:thresholdData,threshold_type:threshold_type});
      }
    });
});
