...
adviceController.get("/danhsachloikhuyen",service.ensureAuthenticated, function(req, res) {
    res.render("expert/loikhuyen/danhsachloikhuyen", {title: 'Xem danh sách lời khuyên',moment:moment,secu:config.securitycode,conf:config.urladdress,token:req.session.token,userid:req.session.userid,fullname:req.session.fullname});
});
adviceController.get("/themloikhuyen",service.ensureAuthenticated,function(req,res){
    res.render("expert/loikhuyen/themloikhuyen",{title: 'Thêm lời khuyên',secu:config.securitycode,conf:config.urladdress,token:req.session.token,userid:req.session.userid,fullname:req.session.fullname})
});
...
adviceController.get("/capnhatloikhuyen/:id",service.ensureAuthenticated,function(req,res){
    ...
    service.get(options,function(error,data){
      if(error){
        ...
        res.render("expert/loikhuyen/capnhatloikhuyen",{title: 'Cập nhật lời khuyên',secu:config.securitycode,conf:config.urladdress,token:req.session.token,userid:req.session.userid,fullname:req.session.fullname,adviceData:adviceData});
      }
    });
});
